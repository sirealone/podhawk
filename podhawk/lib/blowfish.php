...
class Crypt_Blowfish
{
    ...
    function encrypt($plainText)
    {
        if (!is_string($plainText)) {
            die('Blowfish error - Plain text must be a string');
        }
    ...
    function decrypt($cipherText)
    {
        if (!is_string($cipherText)) {
            die('Blowfish error - Cipher text must be a string');
        }
    ...
    {
        if (!is_string($key)) {
            die('Blowfish error - Key must be a string');
        }

        $len = strlen($key);

        if ($len > 56 || $len == 0) {
            die('Blowfish error - Key must be less than 56 characters and non-zero. Supplied key length: ' . $len);
        }
